...
- **🔓 受限内容下载** - 突破Telegram受限频道限制，下载禁止转发的内容（管理员专属）
- **📤 媒体转载** - 转发图片/视频到指定频道（管理员专属）
- **💾 多存储支持** - 支持Local、GDrive、Alist等多种存储位置，文件自动转发
...
| `/wangpan` | 切换存储位置 | `/wangpan` |

### 管理员专属功能

以下功能仅对配置了 `ADMIN_ID` 的管理员开放：

| 功能 | 使用方法 | 说明 |
|------|---------|------|
| **受限内容下载** | 直接发送Telegram链接 | 突破禁止转发限制，下载受限频道内容 |
| **媒体转载** | 转发图片/视频给机器人 | 自动转载到指定频道 |
| **多存储切换** | 使用 `/wangpan` 命令 | 在Local、GDrive等存储间切换 |
